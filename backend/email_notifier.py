...
class EmailNotifier:
    ...
    def send_lost_domain_alert(self, domains):
        """
        Send email alert for lost domains (Available -> Taken)
        """
        if not Config.is_email_configured():
            logger.warning("Email not configured. Skipping notification.")
            return False
        ...
        try:
            # Create message
            msg = MIMEMultipart('alternative')
            msg['Subject'] = f'⚠️ Domain Status Alert: {len(domains)} Domain(s) Taken'
            msg['From'] = config['smtp_from']
            msg['To'] = config['smtp_to']
            
            # Create HTML content
            html_content = self._create_lost_html_email(domains)
            
            # Create plain text version
            text_content = self._create_lost_text_email(domains)
            ...
            logger.info(f"LOST Domain notification sent for {len(domains)} domain(s)")
            return True
            
        except Exception as e:
            logger.error(f"Failed to send LOST domain notification: {e}")
            return False

    def _create_lost_html_email(self, domains):
        """Create HTML email content for lost domains"""
        html = f"""
        <html>
        <head>
            <style>
                ...
                .domain-name {{ font-size: 18px; font-weight: bold; color: #D32F2F; }}
        ...
        <body>
            <div class="header">
                <h1>⚠️ Domain Status Alert</h1>
                <p>Domains have become unavailable (Taken)</p>
            </div>
            <div class="content">
                <p>The following <strong>{len(domains)}</strong> domain(s) have been taken:</p>
        """
        ...
        html += f"""
            ...
            <div class="footer">
                ...
                <p>These domains are no longer available for registration.</p>
            </div>
        </body>
        </html>
        """
    ...
    def _create_lost_text_email(self, domains):
        """Create plain text email content for lost domains"""
        text = f"⚠️ Domain Status Alert: Domains Taken\n"
        text += f"=" * 50 + "\n\n"
        text += f"The following domains are no longer available (Taken): {len(domains)}\n\n"
